...
class ValoracionController extends Controller
{
    ...
    public function export_excel(Request $request)
    {
        ...
        $valoraciones_ids = Valoracion::select('valoracion.id')
                            ...
                            ->join('puntuacion_valoracion', 'puntuacion_valoracion.valoracion_id', '=', 'valoracion.id')
                              ->join('competencia', 'competencia.id', '=', 'puntuacion_valoracion.competencia_id')
                            ->where('valoracion.periodo',  $periodo)
                            ->whereYear('valoracion.created_at', '=', $ano)
                            ->where('competencia.acargo', $acargo)
                            ->get();
      ...
      $spreadsheet->setActiveSheetIndex(0)
        ...
        ->setCellValue("C2", "INDICADORES")
        ->setCellValue("D2", "COMPETENCIAS")
        ->setCellValue("E2", "LIDERAZGO")
        ->setCellValue("F2", "TRABAJO EN EQUIPO")
        ->setCellValue("G2", "INNOVACIÓN");
        ...
        foreach ($valoraciones as $valoracion) {
            ...
            $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(1, $x+2, $valoracion->user->cargo->obj_area->area);
            $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(2, $x+2, $valoracion->user->name.' '.$valoracion->user->apellido);
            ...
            foreach ($valoracion->puntuaciones as $key => $puntuacion) {
                ...
                $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow($key + 8, $x+2, $puntuacion->puntuacion);
            ...
            if($max_total_liderazgo != 0){
                $total_liderazgo = $promedio_liderazgo/$max_total_liderazgo;
            }

            if($max_total_equipo != 0){
                $total_equipo = $promedio_equipo/$max_total_equipo;
            }

            if($max_total_innovacion != 0){
                $total_innovacion = $promedio_innovacion/$max_total_innovacion;
            }

            $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(5, $x+2, number_format((float)$this->FormatAverage($promedio_liderazgo), 2, '.', '').'%');
            $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(6, $x+2, number_format((float)$this->FormatAverage($promedio_equipo), 2, '.', '').'%');
            $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(7, $x+2, number_format((float)$this->FormatAverage($promedio_innovacion), 2, '.', '').'%');

            $indicadores = MaestroIndicadores::where('documento', $valoracion->user->documento)->where('meta', $valoracion->periodo)->whereYear('feha_creacion', $ano)->get();
            ...
            foreach ($indicadores as $key => $indicador) {
                # code...
                $sum_desempeno = $sum_desempeno + $indicador->desempeno;
            ...
            /* */
            $spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(3, $x+2, number_format((float)$total_desempeno.'%') );
            $x++;
            //$y++;
        }
